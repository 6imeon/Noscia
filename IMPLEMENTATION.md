...
- [x] **Per-result summary (Exa-style, BYOK):** `search/summarize.py` distills each top result's own passage into a 1–2 sentence query-focused answer (opt-in `summarize` flag; grounded — `NONE`→null per rule 8; `get_secret` BYOK, no key ⇒ no-op). UI: key-gated ✦ AI-answers toggle, answer rendered above the cited passage.
