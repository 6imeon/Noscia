...
- [x] **Deep crawl (Phase 2.5):** `crawl.py::crawl_site` bounded same-domain BFS (`max_depth`/`max_pages`/exclude per seed); seeds expanded 8 → 21 authoritative domains; `chunks.source_url` + source-level page prune; **581-page / 4,817-chunk** index (incl. PDFs below). Depth, not open-web (rule 11/13).
...
- [x] **PDF extraction (Phase 2.5):** `ingest/pdf.py::extract_pdf` (`httpx` fetch, size-capped, fail-soft → `pypdf` text) pulls authoritative PDFs HTML crawling misses (TCFD reports). `crawl_site` harvests same-domain + per-seed `pdf_hosts`-allow-listed PDF links (curated, not open-web — rule 11/13); `max_pdfs` cap + translation excludes keep it to English originals.
- [x] **URL canonicalization:** `ingest/url.py::canonical_url` folds cosmetic variants (`?page=1`/trailing-slash/`#frag`/`utm_*`/`_gcl*`/`_gl`/`_gac`) at the point a page gets its URL → no duplicate results; conservative (`?page=2` + unknown params preserved).
- [x] **PDF readability:** `pdf.py::_clean_pdf_text` reflows pypdf's layout-split output to prose (rejoin wraps, keep paragraphs + compound hyphens); highlighter treats line breaks as sentence boundaries → no more truncated PDF soup.
- [x] **Synthesized answer (Exa/Perplexity-style, BYOK):** `search/summarize.py::synthesize_answer` makes one grounded LLM call over the top passages → a single 2–4 sentence answer with inline `[n]` citations to result rows (opt-in `summarize` flag; `NONE`→null per rule 8; `get_secret` BYOK, no key ⇒ no-op). UI: key-gated ✦ AI-answers toggle (default on with a key), answer banner above results with citation chips that jump to the row.
- [x] **Recency (Phase 2.5):** publication date extracted at ingest (`crawl.py` head-meta `article:published_time`/Dublin Core/JSON-LD; `pdf.py` PDF `/CreationDate`) → nullable `chunks.published_at` (evidence-or-null, rule 8; page-level `stamp_published_at` backfills incremental recrawls). `search/recency.py::apply_recency_prior` — a **news-only, boost-only**, 180-day-half-life prior (post-rerank, mirrored in eval). **Eval-gated to `MAX_BOOST=0.05`** (rule 9): 0.15 regressed q10 (a fresh esgtoday SBTi piece leapfrogged SBTi's authoritative page, nDCG 0.918→0.893); parity holds ≤0.08, ships at 0.05 (tie-break only) — eval byte-identical to baseline. UI shows the date (row year + reader field). *Caveat:* framework-centric eval proves no-regression, not the prior's benefit — news-relevance eval queries are the next gate.
