...
- [x] **Deep crawl (Phase 2.5):** `crawl.py::crawl_site` bounded same-domain BFS (`max_depth`/`max_pages`/exclude per seed); seeds expanded 8 → 21 authoritative domains; `chunks.source_url` + source-level page prune; **581-page / 4,925-chunk** index (incl. PDFs below). Depth, not open-web (rule 11/13).
...
- [x] **URL canonicalization:** `ingest/url.py::canonical_url` folds cosmetic variants (`?page=1`/trailing-slash/`#frag`/`utm_*`) at the point a page gets its URL → no duplicate results; conservative (`?page=2` + unknown params preserved).
